...
class CalendarAdmin extends React.Component {
  ...
  onSubmitModal = async (e) => {
    ...
    if (obj.title === "") {
      ...
      await API.addAppointmentCambridgePaola(obj)
        .then(() => {
          this.getAppointmentsCalendarAdmin();
          this.setState({
            slotEvent: "",
  ...
  async handleDeleteEvent(id) {
    if (
      ...
      await API.deleteCalendarCambridgePaolaEvent(id)
        .then(() => {
          this.getAppointmentsCalendarAdmin();
          this.setState({
            modalToEditEvent: false,
  ...
  getAppointmentsCalendarAdmin = async () => {
    await API.getAppointmentsCambridgePaola()
      .then((response) => {
  ...
  handleEventGetter = (event) => {
    let id = event.id;
    this.toggleToEdit();
    API.getAppointmentCambridgePaola(id)
      .then((res) => {
        this.setState({
